...
# --trajectory= -2 -3.5,-6 -2.5,-10 -2.5,-18 -2.5,-18 -10.0,-10 -10
# --speed= 1.1
...
class Pedestrian (Supervisor):
    ...
    def add_points(self,array):
        # I added points to achieve the effect that the human remains 
        # static when reaching a point.
        nuevo_array = []
        for i in range(len(array)):
            nuevo_array.append(array[i%len(array)])
            nuevo_array.append(array[(i+1)%(len(array))])
            nuevo_array.append(array[i%len(array)])
        nuevo_array.append(array[0])
        return nuevo_array
    ...
    def run(self):
        ...
        point_list = self.add_points(point_list)
        print(point_list)
